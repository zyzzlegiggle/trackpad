...
function App() {
  ...
  const handleEditorClose = async () => {
    // Restore compact bar window
    try {
      await getCurrentWindow().unmaximize();
      await getCurrentWindow().setDecorations(false);
      await getCurrentWindow().setSize(new LogicalSize(420, 400));
  ...
  useEffect(() => {
    const resizeForEditor = async () => {
      if (editorMode) {
        // Maximize window for editor
        try {
  ...
  // Handle close - must await the promise
  ...
  return (
    <div
      className="w-full h-full p-2 flex flex-col-reverse justify-start"
      style={{ WebkitAppRegion: 'drag' } as React.CSSProperties}
    >
      {/* Main Bar - at bottom due to flex-col-reverse */}
      <div
        className="w-full h-14 bg-white rounded-2xl shadow-lg border border-black/10 flex items-center gap-2 px-3 shrink-0"
        style={{ WebkitAppRegion: 'drag' } as React.CSSProperties}
      >
        {/* Grip dots */}
      ...
      {/* Dropdown Panel - appears ABOVE the bar due to flex-col-reverse */}
      {showSourceModal && (
        <div
          className="mb-2 bg-white rounded-xl shadow-xl border border-gray-200 overflow-hidden"
          style={{ WebkitAppRegion: 'no-drag' } as React.CSSProperties}
        >
          <div className="p-2 border-b border-gray-100 flex items-center justify-between">
            <span className="text-xs font-semibold text-gray-500 uppercase tracking-wide px-2">Select Source</span>
            <button
              type="button"
              onClick={() => setShowSourceModal(false)}
              className="w-6 h-6 border-none bg-transparent rounded-md cursor-pointer flex items-center justify-center text-gray-400 hover:bg-gray-100 hover:text-gray-600"
            >
              ...
              <div className="flex flex-col gap-0.5">
                {windows.map((w) => (
                  <button
                    ...
                    className={`flex items-center gap-2.5 px-3 py-2 rounded-lg cursor-pointer text-left w-full border-none transition-all duration-150 ${selectedTarget === w.id.toString()
                      ? "bg-blue-50 text-blue-700"
                      : "bg-transparent hover:bg-gray-50 text-gray-700"
                      }`}
                  >
                    <div className={`w-7 h-7 rounded-md flex items-center justify-center shrink-0 ${selectedTarget === w.id.toString() ? "bg-blue-100 text-blue-600" : "bg-gray-100 text-gray-500"}`}>
                      <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" className="w-3.5 h-3.5">
